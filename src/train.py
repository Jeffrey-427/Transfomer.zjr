...
@torch.no_grad()
def sample_translations(model, loader, sp, special, lang_tag_id, max_len, device, n=3):
    """从给定 loader 抽 n 条，做 greedy 解码，返回 [(src, ref, hyp), ...]。"""
    model.eval()
    BOS, EOS = special["bos_id"], special["eos_id"]
    out, taken = [], 0
    for batch in loader:
        ...
        for i in range(src.size(0)):
            ids = ys[i, 2:].tolist()       # 去掉 <2xx>, <BOS>
            if EOS in ids: ids = ids[:ids.index(EOS)]
            hyp = sp.decode(ids)
            out.append((batch["raw_src"][i], batch["raw_tgt"][i], hyp))
            taken += 1
            if taken >= n: break
        if taken >= n: break
    return out
...
# ---------- CE on loader ----------
@torch.no_grad()
def evaluate_ce(model, loader, loss_fn, device, special):
    model.eval()
    total, ntok = 0.0, 0
...
def main():
    ...
    for epoch in range(1, cfg["train"]["epochs"] + 1):
        ...
        # —— Dev CE（双向） ——
        dev_en_de = evaluate_ce(model, dev_loader_en_de, loss_fn, device, special)
        dev_de_en = evaluate_ce(model, dev_loader_de_en, loss_fn, device, special)
        ...
        n_show = cfg.get("logging", {}).get("epoch_sample_n", 2)  # 可在 config.yaml 里设置
        if n_show > 0:
            # 目标语言标签：en→de 用 <2de>，de→en 用 <2en>
            lang_id_de = sp.piece_to_id(special["lang_tag_de"])
            lang_id_en = sp.piece_to_id(special["lang_tag_en"])

            ex_en_de = sample_translations(model, dev_loader_en_de, sp, special, lang_id_de,
                                           cfg["infer"]["max_gen_len"], device, n=n_show)
            ex_de_en = sample_translations(model, dev_loader_de_en, sp, special, lang_id_en,
                                           cfg["infer"]["max_gen_len"], device, n=n_show)

            for j, (s, r, h) in enumerate(ex_en_de, 1):
                print(f"[Sample en→de #{j}] SRC: {_clip(s)}")
                print(f"[Sample en→de #{j}] REF: {_clip(r)}")
                print(f"[Sample en→de #{j}] HYP: {_clip(h)}")

            for j, (s, r, h) in enumerate(ex_de_en, 1):
                print(f"[Sample de→en #{j}] SRC: {_clip(s)}")
                print(f"[Sample de→en #{j}] REF: {_clip(r)}")
                print(f"[Sample de→en #{j}] HYP: {_clip(h)}")
