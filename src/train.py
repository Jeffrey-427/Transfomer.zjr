...
@torch.no_grad()
def generate_sample(model, loader, sp, special, device,
                    title=None, max_len=64, num_samples=4, tgt_lang_tag="<2de>"):
    """
    tgt_lang_tag: 目标语言标签的文本（比如 "<2de>" 或 "<2en>"）
    """
    model.eval()
    batch = next(iter(loader))

    # 取一小批样本
    src = batch["src_ids"][:num_samples].to(device)
    src_pad = batch["src_pad"][:num_samples].to(device)

    # 语言标签 + BOS 作为解码前缀（与训练对齐）
    lang_id = sp.piece_to_id(tgt_lang_tag)
    bos_id  = special["bos_id"]
    pad_id  = special["pad_id"]
    eos_id  = special["eos_id"]

    # ys 形状: [B, 2]，每行 [<2xx>, <bos>]
    ys = torch.tensor([[lang_id, bos_id]] * src.size(0), dtype=torch.long, device=device)
    ...
    for _ in range(max_len):
        ...
        L = ys.size(1)
        tgt_sub = torch.tril(torch.ones((L, L), device=device, dtype=torch.bool))
        logits = model.decode(ys, mem, tgt_sub, tgt_pad, src_pad)
    ...
    # 解码文本，清理特殊符号
    src_text = [sp.decode_ids(x.tolist()) for x in batch["src_ids"][:num_samples]]
    tgt_text = []
    for row in ys.tolist():
        ids = []
        for i in row:
            if i in (lang_id, bos_id, pad_id):
                continue
            if i == eos_id:
                break
            ids.append(i)
        tgt_text.append(sp.decode_ids(ids))

    print(f"\n===== {title or 'Samples'} =====")
    for s, t in zip(src_text, tgt_text):
        print(f"SRC: {s}")
        print(f"HYP: {t}")
        print("-" * 40)
...
# ========= 修改 evaluate_ce 定义 =========
@torch.no_grad()
def evaluate_ce(model, loader, loss_fn, device, pad_id):
    model.eval()
    total, ntok = 0.0, 0
    for batch in loader:
        src = batch["src_ids"].to(device)
        ...
        tok_loss = loss_fn(flat_logit, flat_tgt)     # reduction='none'
        mask = (flat_tgt != pad_id)                  # ← 不再引用未定义的 special
...
def main():
    ...
    for epoch in range(1, cfg["train"]["epochs"] + 1):
        ...
        for i, batch in enumerate(train_loader, start=1):
            ...
            scaler.step(opt);
            scaler.update();
            # —— 记录 step 级指标（用于最后三张曲线）——
            step_ce  = float(tok_loss[mask].mean().item())
            step_ppl = math.exp(min(step_ce, 20.0))
            hist_steps.append(global_step)
            hist_loss.append(step_ce)
            hist_ppl.append(step_ppl)
        ...
        # EN→DE 示例
        generate_sample(model, dev_loader_en_de, sp, special, device,
                title="EN → DE Samples", tgt_lang_tag=special["lang_tag_de"])

        # DE→EN 示例
        generate_sample(model, dev_loader_de_en, sp, special, device,
                title="DE → EN Samples", tgt_lang_tag=special["lang_tag_en"])


        # —— Dev CE（双向） ——
        dev_en_de = evaluate_ce(model, dev_loader_en_de, loss_fn, device, pad_id)
        dev_de_en = evaluate_ce(model, dev_loader_de_en, loss_fn, device, pad_id)
